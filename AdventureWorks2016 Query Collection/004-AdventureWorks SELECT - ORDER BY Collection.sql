...
OFFSET @StartingRowNumber ROWS FETCH NEXT @FetchRows ROWS ONLY;
...
FROM   HumanResources.Department
ORDER BY DepartmentID ASC
OFFSET @StartingRowNumber - 1 ROWS FETCH NEXT @EndingRowNumber - @StartingRowNumber + 1 ROWS ONLY OPTION(OPTIMIZE FOR(@StartingRowNumber = 1, @EndingRowNumber = 20));
...
IF
   (
     ...
     FROM   sys.databases
     WHERE  name = N'AdventureWorks2016'
   ) = 0
...
WHILE
    ...
    BEGIN
        ...
        FROM   HumanResources.Department
        ORDER BY DepartmentID ASC
        OFFSET @StartingRowNumber - 1 ROWS FETCH NEXT @RowCountPerPage ROWS ONLY;
